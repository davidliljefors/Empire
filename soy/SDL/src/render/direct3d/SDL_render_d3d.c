...
static bool D3D_QueueDrawPoints(SDL_Renderer *renderer, SDL_RenderCommand *cmd, const SDL_FPoint *points, int count)
{
    ...
    const size_t vertslen = count * sizeof(emp_vertex);
    emp_vertex *verts = (emp_vertex *)SDL_AllocateRenderVertices(renderer, vertslen, 0, &cmd->data.draw.first);
    int i;
                             ...
                             float scale_x, float scale_y)
{
    ...
    emp_vertex *verts = (emp_vertex *)SDL_AllocateRenderVertices(renderer, count * sizeof(emp_vertex), 0, &cmd->data.draw.first);
...
static bool D3D_RunCommandQueue(SDL_Renderer *renderer, SDL_RenderCommand *cmd, void *vertices, size_t vertsize)
{
    ...
    IDirect3DDevice9_SetStreamSource(data->device, 0, vbo, 0, sizeof(emp_vertex));
    ...
    while (cmd) {
        ...
        case SDL_RENDERCMD_DRAW_LINES:
        {
            ...
            const size_t start = first / sizeof(emp_vertex);
            const emp_vertex *verts = (emp_vertex *)(((Uint8 *)vertices) + first);
            ...
            if (count == 2 || verts[line_start].x != verts[line_end].x || verts[line_start].y != verts[line_end].y) {
                if (vbo) {
                    ...
                    IDirect3DDevice9_DrawPrimitiveUP(data->device, D3DPT_POINTLIST, 1, &verts[line_end], sizeof(emp_vertex));
                }
            }
            ...
            if (count > 2) {
                ...
                if (vbo) {
                    ...
                    IDirect3DDevice9_DrawPrimitiveUP(data->device, D3DPT_LINESTRIP, (UINT)(count - 1), verts, sizeof(emp_vertex));
                }
            } else {
                ...
                if (vbo) {
                    ...
                    IDirect3DDevice9_DrawPrimitiveUP(data->device, D3DPT_LINELIST, (UINT)(count - 1), verts, sizeof(emp_vertex));
                }
        ...
        case SDL_RENDERCMD_GEOMETRY:
        {
            ...
            if (thiscmdtype == SDL_RENDERCMD_GEOMETRY) {
                if (vbo) {
                    ...
                    IDirect3DDevice9_DrawPrimitiveUP(data->device, D3DPT_TRIANGLELIST, (UINT)count / 3, verts, sizeof(emp_vertex));
                }
            } else {
                if (vbo) {
                    IDirect3DDevice9_DrawPrimitive(data->device, D3DPT_POINTLIST, (UINT)start, (UINT)count);
                } else {
                    IDirect3DDevice9_DrawPrimitiveUP(data->device, D3DPT_POINTLIST, (UINT)count, verts, sizeof(emp_vertex));
                }
            }
            cmd = finalcmd; // skip any copy commands we just combined in here.
            break;
