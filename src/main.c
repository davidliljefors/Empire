...
static void emp_material_bind(emp_material_t const* material) {
	...
	for (size_t index = 0; index < material->ubo.count; index++) {
		...
		case emp_uniform_int:
			glUniform1i(g_has_texture_loc, *(GLint*)uniform->stable_data);
			break;
		}
	}
}
...
int main(int argc, char* argv[]) {
	...
	emp_gpu_instance_t gpu_instances[512];
	memset(gpu_instances, 0, sizeof(gpu_instances));

	emp_gpu_state_t gpu;
	memset(&gpu, 0, sizeof(gpu));

	emp_material_t* material = &gpu.material;
